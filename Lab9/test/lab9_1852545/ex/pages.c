...
int translate(address_t virtual, address_t * physical) {
	...
	//get index of page needed and offset
	index_t page_index = get_index(virtual);
	offset_t offset = get_offset(virtual);
	
	//Check tlb first for tlb hit 
	for(int i = 0; i< TLB_SIZE; i++){
		if(tlb[i].virtual == page_index){ 
			tlb[i].count++; // increase access count
			*physical = (tlb[i].physical << OFF_LEN) | offset;
			return 1;
			//found in tlb
		}
	}	
	//If tlb miss, go to page table
	struct tlb_slot_t * least_used_slot = get_least_accessed_slot();
	for(int i = 0; i< PAGE_TABLE_SIZE; i++){
		if(page_table[i].virtual == page_index){
			least_used_slot->count = 0;
			*least_used_slot = page_table[i]; // update tlb
			least_used_slot->count++;
			*physical = (page_table[i].physical << OFF_LEN) | offset;
			return 1;
			//found in page table
		}
	}
	//Can't find in page table (invalid)
	return 0;
